    ...
    <>
      ...
      {mainTab === "order" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <OrderTab masterShops={masterShops} locationNames={locationNames} />
        </div>
      )}
...
function OrderTab({ masterShops, locationNames }) {
  const [orders, setOrders] = useState(() => {
    const s = localStorage.getItem("frp_orders");
    return s ? JSON.parse(s) : {};
  });
  const [activeLocTab, setActiveLocTab] = useState(1);

  const shops = masterShops[activeLocTab] || [];
  const getTotal = (shopId) => Number(orders[shopId] || 0);

  const addAmount = (shopId, amount) => {
    if (!amount || isNaN(amount)) return;
    setOrders(prev => {
      const next = { ...prev, [shopId]: (Number(prev[shopId] || 0)) + Number(amount) };
      localStorage.setItem("frp_orders", JSON.stringify(next));
      return next;
    });
  };

  const resetShop = (shopId, shopName) => {
    if (!confirm(`Reset orders for ${shopName}?`)) return;
    setOrders(prev => {
      const next = { ...prev, [shopId]: 0 };
      localStorage.setItem("frp_orders", JSON.stringify(next));
      return next;
    });
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", background: "#080d14" }}>
      {/* Loc tabs */}
      <div style={{
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px 80px" }}>
        <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: "#374151", padding: "4px 2px 10px", letterSpacing: 1.5 }}>
          MONTHLY ORDERS · TARGET ₹1,000 / SHOP
        </div>

        {shops.length === 0 ? (
          <div className="empty" style={{ paddingTop: 40 }}>
            <div className="empty-icon">📦</div>
            <div className="empty-text">No shops here yet.<br />Add them from the Route tab.</div>
          </div>
        ) : shops.map(shop => {
            ...
            <div key={shop.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                <div style={{
                  ...
                  background: hit ? "#10b981" : "#1f2937", flexShrink: 0,
                }} />
                <div style={{
                  fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700,
                  fontSize: 14, color: hit ? "#10b981" : "#f3f4f6",
                  flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                }}>{shop.name}</div>
                <div style={{
                  fontFamily: "'JetBrains Mono',monospace", fontSize: 14,
                  fontWeight: 700, color: hit ? "#10b981" : "#f97316",
                }}>₹{total.toLocaleString()}</div>
              </div>

              {/* Progress bar */}
              <div style={{ height: 3, background: "#111827", borderRadius: 4, marginBottom: 10 }}>
                <div style={{
                ...
                <input
                  ...
                  onKeyDown={e => {
                    if (e.key === "Enter" && e.target.value) {
                      addAmount(shop.id, e.target.value);
                      e.target.value = "";
                    }
                  }}
                />
                <button onClick={() => {
                  const input = document.getElementById(`order-${shop.id}`);
                  addAmount(shop.id, input.value);
                  input.value = "";
                }} style={{
                  background: "#f97316", border: "none", color: "#080d14",
                  borderRadius: 8, padding: "8px 14px", cursor: "pointer",
                  fontFamily: "'Space Grotesk',sans-serif", fontSize: 12, fontWeight: 700,
                }}>+ Add</button>
                <button onClick={() => resetShop(shop.id, shop.name)} style={{
                  background: "#1a0d0d", border: "none", color: "#f87171",
